...
class TextsRU(StrEnum):
    ...
    ADMIN_BAN_USER_PROMPT = (
        ...
        "Вы можете узнать ID пользователя, попросив его отправить команду /start боту @userinfobot"
    )
    ADMIN_BAN_USER_REASON_PROMPT = (
        "Введите причину блокировки пользователя {user_id}.\n\n"
        "Если причина не требуется, отправьте любой текст."
    )
    ADMIN_BAN_USER_INVALID_ID = "❌ Некорректный ID пользователя. Введите числовой ID."
    ADMIN_BAN_USER_SELF_BAN = "❌ Вы не можете заблокировать самого себя!"
    ADMIN_BAN_USER_SUCCESS = "✅ Пользователь {user_id} заблокирован."
    ADMIN_BAN_USER_FAILED = "❌ Не удалось заблокировать пользователя. Возможно, он не существует в системе."
    ADMIN_UNBAN_USER_PROMPT = (
        "Введите ID пользователя для разблокировки.\n\n"
        "Вы можете узнать ID пользователя, попросив его отправить команду /start боту @userinfobot"
    )
    ADMIN_UNBAN_USER_INVALID_ID = (
        "❌ Некорректный ID пользователя. Введите числовой ID."
    )
    ADMIN_UNBAN_USER_SUCCESS = "✅ Пользователь {user_id} разблокирован."
    ADMIN_UNBAN_USER_NOT_BANNED = "ℹ️ Пользователь {user_id} не был заблокирован."
    ADMIN_UNBAN_USER_FAILED = "❌ Не удалось разблокировать пользователя."
